...
for bam, name in zip(files, filenames):
    ...
    #bam to sam
    infileHeader = pysam.Samfile(bam, mode='rb').header
    infile = pysam.Samfile(bam, mode='rb', header=infileHeader)
    outfile = pysam.Samfile(name+"_temp"+bamid+".sam", "wh", 
    ...
    #get read 1 5' with read 2 directionality
    result=open(name+"_snr_temp"+bamid+".sam",'w')
    flags=["83", "99"] # changed these to first strand flags
        ...
        for line in infile:
            if not line.startswith("@"):
                ...
                if line[1] in flags and line[6]=="=" and \
                    ...
                    if line[1]==flags[0]:
                        # line[1]="99" #*** 99 or 83 in case of wrong strand
                        line[1]="83" 
                        # line=setStrand(line) #*** remove in case of wrong strand
                        add_to_start=readLength_CIGAR(line[5])
                        line[3]=str(int(line[3])+add_to_start)
                        # line[9]=line[9][-1] #*** -1 or 0 in case of mismatched bases
                        # line[10]=line[10][-1] #*** -1 or 0 in case of mismatched bases
                        line[9]=line[9][0] 
                        line[10]=line[10][0] 
                        line[5]="1M"

                    elif line[1]==flags[1]:
                        # line[1]="83" #*** 99 or 83 in case of wrong strand
                        # line=setStrand(line) #*** remove in case of wrong strand
                        # line[9]=line[9][0] #*** -1 or 0 in case of mismatched bases
                        # line[10]=line[10][0] #*** -1 or 0 in case of mismatched bases
                        line[1]="99"
                        line[9]=line[9][-1]
                        line[10]=line[10][-1]
                        line[5]="1M"
    ...
    infile = pysam.Samfile(name+"_snr_temp"+bamid+".sam", "r")
    outfile = pysam.Samfile(name+"_snr_temp"+bamid+".bam", 
        "wb", template=infile)
    for s in infile:
        outfile.write(s)
    infile.close()
