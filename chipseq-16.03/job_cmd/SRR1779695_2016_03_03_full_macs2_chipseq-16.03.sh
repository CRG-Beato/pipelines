#!/bin/bash
#$ -N SRR1779695_2016_03_03_full_macs2_chipseq-16.03
#$ -q long-sl65
#$ -l virtual_free=40G
#$ -l h_rt=20:00:00
#$ -o /users/GR/mb/jquilez/pipelines/chipseq-16.03/job_out/SRR1779695_2016_03_03_full_macs2_chipseq-16.03_$JOB_ID.out
#$ -e /users/GR/mb/jquilez/pipelines/chipseq-16.03/job_out/SRR1779695_2016_03_03_full_macs2_chipseq-16.03_$JOB_ID.err
#$ -j y
#$ -M javier.quilez@crg.eu
#$ -m abe
#$ -pe smp 8

submitted_on=2016_03_03
pipeline_version=16.03
sample_id=SRR1779695
data_type=atacseq
...
use_control=no
macs2_qvalue=0.05
control_bam=/users/GR/mb/jquilez/data/chipseq/samples/T0_INPUT_F/alignments/bwa/hg19/T0_INPUT_F_sorted_unique.bam
...
main() {
	...
	if [[ $integrate_metadata == "yes" ]]; then
		$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PIPELINE_RUN_MODE -v $pipeline_run_mode
		$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a QUEUE -v $queue
		$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a MEMORY -v $memory
		$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a MAX_TIME -v $max_time
		$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a SLOTS -v $slots
		$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a ASSEMBLY_VERSION -v $version		
	fi
...
message_time_step() {
	...
	if [[ $integrate_metadata == "yes" ]]; then
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a $field_name -v $length
	fi
	echo
}
...
trim_reads_trimmomatic() {
	...
	if [[ $sequencing_type == "SE" ]]; then
		if [ -f $ifq1 ]; then
			...
			ODIR=$SINGLE
			if [[ $integrate_metadata == "yes" ]]; then
				$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PATH_FASTQ_READ1 -v $ifq1
		...
		if [ -f $ifq1 ] && [ -f $ifq2 ]; then
			...
			if [[ $integrate_metadata == "yes" ]]; then
				$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PATH_FASTQ_READ1 -v $ifq1
				$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PATH_FASTQ_READ2 -v $ifq2
	...
	# update metadata
	if [[ $integrate_metadata == "yes" ]]; then
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a ADAPTERS_SEQS -v $seqs
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a SEED_MISMATCHES -v $seedMismatches
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PALINDROME_CLIP_THRESHOLD -v $palindromeClipThreshold
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a SIMPLE_CLIP_THRESHOLD -v $simpleClipThreshold
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a MIN_ADAPTER_LENGTH -v $minAdapterLength
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a KEEP_BOTH_READS -v $keepBothReads
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a LEADING -v $leading
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a TRAILING -v $trailing
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a TARGET_LENGTH -v $targetLength
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a STRICTNESS -v $strictness
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a MIN_LENGTH -v $minLength
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a N_READS_TRIMMED -v $n_reads_trimmed
...
align_bwa() {
	...
	# update metadata
	if [[ $integrate_metadata == "yes" ]]; then
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a GENOME_FASTA -v $genome_fasta
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a N_READS_ALIGNED -v $n_reads_aligned
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a N_READS_UNIQUE -v $n_reads_unique
...
make_tag_directory() {
	...
	# update metadata
	if [[ $integrate_metadata == "yes" ]]; then
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a ESTIMATED_GENOME_SIZE -v $estimated_genome_size
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a ESTIMETED_AVERAGE_READ_DENSITY_PER_BP -v $estimated_average_read_density_per_bp
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a TOTAL_TAGS -v $total_tags
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a TOTAL_POSITIONS -v $total_positions
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a AVG_TAG_LENGTH -v $avg_tag_length
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a MEDIAN_TAGS_PER_POSITION -v $median_tags_per_position
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a AVG_TAGS_PER_POSITION -v $avg_tags_per_position
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a FRAGMENT_LENGTH_ESTIMATE -v $fragment_length_estimate
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PEAK_WIDTH_ESTIMATE -v $peak_width_estimate
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a AUTOCORRELATION_SAME_STRAND_FOLD_ENRICHMENT -v $autocorrelation_same_strand_fold_enrichment
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a AUTOCORRELATION_DIFF_STRAND_FOLD_ENRICHMENT -v $autocorrelation_diff_strand_fold_enrichment
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a AUTOCORRELATION_SAME_TO_DIFF_STRAND_FOLD_ENRICHMENT -v $autocorrelation_same_to_diff_strand_fold_enrichment
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a AVG_FRAGMENT_GC -v $avg_fragment_gc
	 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a AVG_EXPECTED_GC -v $avg_expected_gc
	...
	if [[ $peak_caller == "macs2" ]]; then
		...
		if [[ $use_control == "yes" ]]; then
			...
			message_info $step "peaks = $n_peaks"
	 		...
	 		# update metadata
		 	if [[ $integrate_metadata == "yes" ]]; then
			 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PEAK_CALLER -v $peak_caller
			 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a QVALUE_CUTOFF -v $macs2_qvalue
			 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a PATH_CONTROL_BAM -v $control_bam
			 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a FRAGMENT_LENGTH_ESTIMATE_CORRECTED -v $fragment_length_estimate_corrected
			 	$io_metadata -m add_to_metadata -t 'jobs' -s $sample_id -u $run_date -j $job_id -a N_PEAKS -v $n_peaks
...
data_integrity() {
	...
	# make SHA diggest of the most relevant file (e.g. input FASTQ, alignments BAM, peaks)
	message_info $step "make SHA diggest of the most relevant file (e.g. input FASTQ, alignments BAM, peaks)"
