#!/bin/bash
#$ -N test_2016_01_19_quantification_featurecounts_rnaseq-20160119
#$ -q short-sl65
#$ -l virtual_free=1G
#$ -l h_rt=06:00:00
#$ -o /users/GR/mb/jquilez/pipelines/rnaseq-20160119/job_out/test_2016_01_19_quantification_featurecounts_rnaseq-20160119_$JOB_ID.out
#$ -e /users/GR/mb/jquilez/pipelines/rnaseq-20160119/job_out/test_2016_01_19_quantification_featurecounts_rnaseq-20160119_$JOB_ID.err
#$ -j y
#$ -M javier.quilez@crg.eu
#$ -m abe
#$ -pe smp 4

submitted_on=2016_01_19
pipeline_version=20160119
sample_id=test
data_type=rnaseq
pipeline_run_mode=quantification_featurecounts
io_mode=standard
CUSTOM_IN=$HOME/pipelines/rnaseq-20160119/test
...
n_bootstraps=10
strand_specific=2
PIPELINE=/users/GR/mb/jquilez/pipelines/rnaseq-20160119
config=pipelines/rnaseq-20160119/rnaseq.config
time_start=$(date +"%s")
...
HTSEQ_QUANT=$SAMPLE/quantifications/htseq/$version
...
chrom_sizes=$HOME/assemblies/$species/$version/ucsc/$version.chrom.sizes.autosomes.chrX
if [[ $version == "hg19" ]]; then
	...
	transcripts_gtf=$HOME/assemblies/$species/$version/gencode/gencode.v19.annotation.gtf
elif [[ $version == "hg38" ]]; then
	kallisto_index=$HOME/assemblies/$species/$version/kallisto_index/kallisto_${species}_${version}_gencode_v24.index
	transcripts_gtf=$HOME/assemblies/$species/$version/gencode/gencode.v24.annotation.gtf
fi
...
quantification_featurecounts() {
	...
	# quantify abundances of transcripts
	# -p = fragments will be counted instead of reads (2 reads per fragment)
	# -g transcript_id = summarize reads counts per transcript
	# -O = reads/fragments are allowed to be assigned to more than one transcript
	...
	message_info $step "kallisto (on transcripts) and featurecounts (on genes) quantifications are not directly comparable"
...
make_profiles() {
	...
	if [ -d $STAR ]; then
		...
		message_info $step "profiles are in $PROFILES"
