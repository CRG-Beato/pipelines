#!/bin/bash
#$ -N rnaseq_20151218_full_2015_12_21_100
#$ -q short-sl65
#$ -l virtual_free=10G
#$ -l h_rt=06:00:00
#$ -o /users/GR/mb/jquilez/pipelines/rnaseq-20151218/job_out/rnaseq_20151218_full_2015_12_21_100_$JOB_ID.out
#$ -e /users/GR/mb/jquilez/pipelines/rnaseq-20151218/job_out/rnaseq_20151218_full_2015_12_21_100_$JOB_ID.err
#$ -j y
#$ -V
...
make_profiles() {
	...
	if [ -d $STAR ]; then
		...
		obed=$PROFILES/${sample_id}_star_alignments.bed
		...
		obb=$PROFILES/${sample_id}_star_alignments.bb
	...
	if [ -d $KALLISTO_ALIGN ]; then
		
		# make read profiles from kallisto pseudoalignments		
		# filter valid pairs (for paired-end data only) and convert BAM to BED
		# valid pairs are retained with samtools's -f 0x2 command
		message_info $step "make read profiles from kallisto pseudoalignments"
		message_info $step "Filtering valid pairs (for paired-end data only) and convert BAM to BED"
		ibam=$KALLISTO_ALIGN/${sample_id}_pseudoalignment.bam
		obed=$PROFILES/${sample_id}_kallisto_pseudoalignments.bed
		if [[ $sequencing_type == "PE" ]]; then
			$samtools view -bf 0x2 $ibam | $bamToBed -i stdin > $obed
		else
			$bamToBed -i $ibam > $obed
		...
		# convert BED to bigWig (more suitable for UCSC Genome Browser uploads)
		message_info $step "converting BED to bigWig (more suitable for UCSC Genome Browser uploads)"
		obb=$PROFILES/${sample_id}_kallisto_pseudoalignments.bb
		$bed2bb $obed $chrom_sizes $obb >$log_make_profiles 2>&1
